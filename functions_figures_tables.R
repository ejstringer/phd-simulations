...
em.conceptual_phase <- function(startdate = "2007-01-01",
  ...
  if(syoung) rain_caps$captures <- rain_caps$capturesSy
  ...
  phaseVis <- rbind(subset_data_visualise,
    ...
    filter(subset != 'L1')
  if(syoung){
    phaseVis$species <- ifelse(phaseVis$species == 'capturesSy', 
                               'captures', 'capturesSy')
  }
  ...
  if(!syoung){
    conceptual <- conceptual+
      geom_text(data = sdv[-1,], 
                aes(x = trip, y = rain + 7,label = period),
                hjust = -0.5, size = 4.5)
  }
  return(conceptual)
}
