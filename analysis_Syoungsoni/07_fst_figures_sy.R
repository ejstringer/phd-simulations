...
phaseNo <- paste0(rep(c('L','I', 'D'), 3), rep(1:3, each = 3))[-1]
 ...
 vis<-  data.frame(gen = c(rep(which(!duplicated(dfs$phaseNo)), each = 4)-0.5, 11,11)[-c(1,2)],
             phaseNo = rep(phaseNo, each = 4),
             lower = 0,
             upper = 0.04,
             fst = c(0, rep(rep(c(0.017, 0), each = 2), 8))[-32]) %>% 
...
conditionsx <-conditions %>% rename(Nmax = N) %>% 
  group_by(gen, phaseNo, phase, Nmax, migration) %>% 
