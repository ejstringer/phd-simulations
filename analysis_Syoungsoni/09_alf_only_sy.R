...
dir.create('/data/scratch/emily/simulations/sy/alf')
saveRDS(alfReal, '/data/scratch/emily/simulations/sy/alf/real_alf.rds')
...
alf_nSim <- foreach(i = 1:20) %dopar% {
  ...
  for(j in 1:50) {
    ...
    alffilename <- sub('\\.rds', paste0('_alf_n', sprintf("%02d", j), '.rds'),
                       filename)
    alf_list <- list.files('/data/scratch/emily/simulations/sy/alf/')
    ...
    nSimalf$i <- i+(j/100)
    saveRDS(nSimalf, paste0('/data/scratch/emily/simulations/sy/alf/', 
                   alffilename))
    
    nSim_list[[j]] <- nSimalf 
    }else{
      nSim_list[[j]] <- readRDS(paste0('/data/scratch/emily/simulations/sy/alf/', 
                                  alffilename))
    }
  }
...
# -------
models_random <- foreach(i = 1:1000, .combine='rbind') %dopar% {
  library(tidyverse)
  alfdf <- alfdf[-which(names(alfdf) == 'npp')]
  random$npp <-sample(random$nppTrue, 9) 
  alfdf <- left_join(alfdf, random)
  alfdf$npp.log <- log(alfdf$npp)
  
alfloci <- split(alfdf, alfdf$loci)
system.time(m.alfnpp <- lapply(alfloci, em.alf_npp_model, iteration = 'real') %>% 
              do.call('rbind', .))

round(c(r = cor(random$nppTrue[-1], random$npp[-1]), n = sum(m.alfnpp$pvalue < 0.05)),2)
}

stopCluster(cl)
saveRDS(models_random, './random_npp_cor_npvalue05.rds')
models_random %>% 
  as.data.frame %>% 
  mutate(type = 'random') %>% 
  ggplot(aes(n)) +
  geom_histogram(colour = 'black',
                 fill = 'lightgreen', bins =)+
  theme_classic()
